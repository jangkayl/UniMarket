...
const Navbar = () => {
  ...
  // Navigation links for the authenticated state (Removed Messages & Notifications)
  const navLinks = [
  ...
  return (
    <header className="bg-red-900 text-white py-4 px-8 flex justify-between items-center w-full sticky top-0 z-50 shadow-md">
      {/* Logo Section - Always visible, clickable to Home */}
      <Link
        href="/"
      ...
      {isPublicPage ? (
        /* --- PUBLIC STATE (Landing, Login, Register) --- */
        <div>
          <Link
            href="/help"
        ...
        /* --- APP STATE (Dashboard, Marketplace, etc.) --- */
        <>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-sm transition-colors hover:text-white ${
                  pathname === link.href
                    ? "text-white font-bold border-b-2 border-white pb-1" // Active State
                    : "text-red-100 font-medium" // Inactive State
                }`}
              >
                {link.name}
              </Link>
          ...
          <div className="flex items-center gap-6">
            ...
            {/* Logout Button */}
            <Link href="/login">
              <button className="flex items-center gap-2 bg-white text-red-900 px-4 py-2 rounded-lg text-sm font-bold hover:bg-gray-100 transition-colors shadow-sm cursor-pointer">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
